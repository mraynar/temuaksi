...
class EditKegiatanVolunteerPage extends StatefulWidget {
  ...
  const EditKegiatanVolunteerPage({super.key, required this.docId, required this.data});

  @override
  State<EditKegiatanVolunteerPage> createState() => _EditKegiatanVolunteerPageState();
}

class _EditKegiatanVolunteerPageState extends State<EditKegiatanVolunteerPage> {
  ...
  void initState() {
    ...
    _kuotaController.text = (widget.data['kuota'] ?? '').toString();
    _persyaratanController.text = widget.data['persyaratan'] ?? widget.data['requirements'] ?? '';
    _selectedKategori = widget.data['kategori'] ?? 'Lingkungan';
    ...
    final jamStr = widget.data['jam_mulai'] as String?;
    if (jamStr != null && jamStr.contains(':')) {
      final parts = jamStr.split(':');
      _jamMulai = TimeOfDay(hour: int.parse(parts[0]), minute: int.parse(parts[1]));
    }
  }
  ...
  Widget _buildPhotoPicker() {
    ...
    if (_selectedImage != null) {
      ...
      imageWidget = Image.network(_existingPhotoUrl, fit: BoxFit.cover);
    } else {
      imageWidget = Column(
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Stack(
          ...
          children: [
            imageWidget,
            if (_selectedImage != null || _existingPhotoUrl.isNotEmpty)
              Positioned(
                top: 8,
                right: 8,
